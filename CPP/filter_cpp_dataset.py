...
# C++ file extensions to consider
CPP_EXTENSIONS = {".cpp", ".h", ".hpp", ".cc", ".cxx", ".hh", ".c"}

# Regex to find class, struct, or namespace declarations in diff lines
# It matches 'class Name', 'struct Name', 'namespace Name'
CLASS_DECL_RE = re.compile(r"\b(?:class|struct|namespace)\s+([a-zA-Z0-9_]+(?:::[a-zA-Z0-9_]+)*)\b")
...
def get_class_from_path(path_text: str) -> str | None:
    ...
    if path.suffix.lower() not in CPP_EXTENSIONS:
        return None
    stem = path.stem
    # Basic heuristic: if it starts with an uppercase letter, it's likely a class-related file
    if stem and (stem[0].isupper() or "_" in stem):
        return stem
    return None

def parse_diff_classes(diff_text: str) -> set[str]:
    classes_in_diff: set[str] = set()
    current_file_class: str | None = None
    ...
    for line in diff_text.splitlines():
        ...
        # Only look at added or context lines for declarations to avoid picking up deleted ones
        # though usually we want anything that defines the scope of change
...
def is_class_mentioned(message: str, class_full_name: str, case_sensitive: bool = False) -> bool:
    flags = 0 if case_sensitive else re.IGNORECASE
    
    # 1. Full name match
    if re.search(re.escape(class_full_name), message, flags):
        return True
    
    # 2. Short name match (if it has namespaces)
    if "::" in class_full_name:
        short_name = class_full_name.split("::")[-1]
        # Use word boundaries to avoid partial matches
        pattern = r"\b" + re.escape(short_name) + r"\b"
        if re.search(pattern, message, flags):
            return True
            
    # 3. For camelCase or snake_case, also try word boundaries
    pattern = r"\b" + re.escape(class_full_name) + r"\b"
    if re.search(pattern, message, flags):
        return True
...
def main():
    parser = argparse.ArgumentParser(description="Unified C++ dataset filter and labeler.")
    parser.add_argument("--input", default="full_cpp_projects.jsonl", help="Input JSONL path")
    parser.add_argument("--output", default="full_cpp_balanced.jsonl", help="Output JSONL path")
